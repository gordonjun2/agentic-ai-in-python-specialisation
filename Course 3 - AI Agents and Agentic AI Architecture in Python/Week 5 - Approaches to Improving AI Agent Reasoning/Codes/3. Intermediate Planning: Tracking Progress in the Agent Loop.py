...
class PlanFirstCapability(Capability):
    ...
    def __init__(self, plan_memory_type="system", track_progress=False):
        super().__init__(
            ...
            description=
            "The Agent will always create a plan and add it to memory")
...
class ProgressTrackingCapability(Capability):
    ...
    def end_agent_loop(self, agent, action_context: ActionContext):
        ...
        progress_report = track_progress(action_context=action_context,
                                         _memory=memory,
        ...
        memory.add_memory({
            ...
            self.memory_type,
            "content":
            f"Progress Report (Iteration {self.iteration_count}):\n{progress_report}"
        })
...
def create_plan(action_context: ActionContext, memory: Memory,
...
def track_progress(action_context: ActionContext, _memory: Memory,
                   action_registry: ActionRegistry) -> str:
