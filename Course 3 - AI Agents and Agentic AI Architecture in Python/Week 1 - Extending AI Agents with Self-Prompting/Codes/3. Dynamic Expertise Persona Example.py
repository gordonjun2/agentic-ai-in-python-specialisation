...
def create_and_consult_expert(action_context: ActionContext,
    ...
    persona_description = generate_response(
        Prompt(messages=[{
            "role": "user",
            "content": persona_description_prompt
        }]))
    ...
    consultation_prompt = generate_response(
        Prompt(messages=[{
            "role": "user",
            "content": consultation_prompt_generator
        }]))
...
def create_dynamic_agent():
    ...
    goals = [
        Goal(priority=1,
             name="Explain Real-World Processes Clearly",
             description="""
            Your goal is to:
            1. Consult a domain expert using the `create_and_consult_expert` function when needed.
            2. Always use the `create_and_consult_expert` function before termination.
            2. Clearly explain real-world processes step by step.
            3. Use structured, concise, and easy-to-follow language.
            4. Terminate after producing a clear explanation.
            """)
    ]
...
def main():
    ...
    user_text = """
    Explain the process of how Jollibee prepares and serves food from kitchen to customer.
    """
